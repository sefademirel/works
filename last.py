...
def create_commit_and_push_branches(repo_path, first_column_names, second_column_names, third_column_names, folders_path, remote_name='origin'):
    ...
    if not os.path.isdir(folders_path):
        raise FileNotFoundError(f"Kontrol edilecek klasör dizini bulunamadı: {folders_path}")
    ...
    def create_commit_push(branch_name, parent_branch=None):
        if branch_name not in existing_branches:
            ...
            if result.returncode == 0:
                ...
                # Üçüncü sütundaki verilerle eşleşen klasörlerin içeriğini commit et
                folder_name = format_branch_name(branch_name)
                ...
                if os.path.isdir(folder_path):
                    for root, dirs, files in os.walk(folder_path):
                        for file in files:
                            ...
                            subprocess.run([git_path, 'add', file_path], check=True)
                    
                    subprocess.run([git_path, 'commit', '-m', f"Added files from folder {folder_name}"], check=True)
                    ...
                    print(f"Folder '{folder_name}' bulunamadı.")
            else:
...
def main():
    ...
    folders_path = r'C:\Users\sefad\OneDrive\Masaüstü\cc'  # Klasörlerin bulunduğu yol
    ...
    create_commit_and_push_branches(repo_path, first_column_names, second_column_names, third_column_names, folders_path)
